...
if sys.argv[2] == "accountlist":
	...
	if fromaccount.commodity != toaccount.commodity:
		# TODO: maybe introduce an override switch, we could still do this if the
		# user really really wants it...
	...
	for transaction in data.transactions.values():
		for split in transaction.splits:
			if split.account == fromaccount:
				...
				gnucash.change_split_account(conn, split.guid, fromaccount.guid, toaccount.guid)
else:
